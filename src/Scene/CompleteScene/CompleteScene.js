...
export default function CompleteScene() {
  ...
  return (
    <div>
      ...
      <div className={"complete_nextbutton"}>
      </div>
      <div className={"complete_finishbutton"}>
      </div>

      <button className="complete_nextbutton" onClick={() => navigate("/NewIndex")} style={{ background: "none", border: "none", padding: 0 }}>
        <img src={img_finish_nextbutton} alt="Input Link Button" />
      </button>

      <button className="complete_finishbutton" onClick={() => navigate("/Result")} style={{ background: "none", border: "none", padding: 0 }}>
